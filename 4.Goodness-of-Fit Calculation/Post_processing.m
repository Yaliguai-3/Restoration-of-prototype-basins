...
% --- 定义文件名  ---
file_ctrl_points = 'Corrected_Control_Points.xlsx';   % 原表1
file_model_data  = 'Model_Calculation_Data.xlsx';     % 原表2
file_uplift      = 'Uplift_Flexure_Params.xlsx';      % 新表
file_output      = 'Post_Process_Results.xlsx';       % 输出结果
...
col_A = T1_raw(:, 1);          % 恢复率
col_B = round(T1_raw(:, 2));   % Te (取整)
col_CG = round(T1_raw(:, 3:7));% X数值 (取整)
