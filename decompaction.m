...
%% 第一步：生成测试数据文件 
% Excel文件名为 'input_data.xlsx' 或修改下方 filename 变量
...
% 获取列数据的引用 (根据列的位置，A=1, B=2, C=3, D=4, E=5)
% 即使列名不一样，只要顺序对(A-E)即可
col_well = raw_data{:, 1};       % A列 井名
col_layer = raw_data{:, 2};      % B列 地层名
col_thick = raw_data{:, 3};      % C列 厚度 (m)
col_phi = raw_data{:, 4};        % D列 孔隙度 (小数)
col_rho = raw_data{:, 5};        % E列 密度
...
compaction_c = zeros(num_rows, 1); % 记录计算出的压实系数供参考

% 默认初始孔隙度
phi_0 = 0.5; 

%% 第三步：逐层计算去压实
% 逻辑：遍历每一行，检测井名变化来重置深度累计

current_well = '';
current_depth_bottom = 0; % 当前累积底界深度
...
for i = 1:num_rows
    ...
    current_depth_bottom = current_depth_bottom + h_now; % 更新底深
    mid_depth = top_depth + (h_now / 2);
    
    % 2. 计算骨架厚度 (H_solid) - 这是一个守恒量
    ...
    % 防止 c 算出负数（如果输入的现今孔隙度比初始还大，这在压实中不合理）
    if c_val < 1e-6
        c_val = 1e-6; 
    end
...
output_table.Restored_Thickness_m = round(restored_thickness, 2); % 恢复后厚度
output_table.Decompacted_Amount_m = round(restored_amount, 2);    % 去压实恢复量
output_table.Calc_Compaction_Factor_c = compaction_c;             % 计算用的压实系数
